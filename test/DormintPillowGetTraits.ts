// import { DormintPillowsGetTraits } from './../typechain-types/contracts/DormintGetTraits.sol/DormintPillowsGetTraits';
import { loadFixture } from "@nomicfoundation/hardhat-network-helpers";
import { expect } from "chai";
import { ethers, upgrades } from "hardhat";

import { DormintPillowsGetTraits } from "./../typechain-types";

describe("DormintPillowsGetTraits", function () {
  async function prepare() {
    ...
    // Deploy NFT
    const DormintPillowsGetTraits = await ethers.getContractFactory(
      "DormintPillowsGetTraits"
    );
    const nft = <DormintPillowsGetTraits>(
      await upgrades.deployProxy(DormintPillowsGetTraits, [
        governor.address,
        mockToken.address,
  ...
  describe("Deployment", function () {
    it("Should be correctly initialized", async function () {
      const { owner, governor, nft, DEFAULT_ADMIN_ROLE } = await loadFixture(
        prepare
      );
      ...
      expect(await nft.hasRole(DEFAULT_ADMIN_ROLE, governor.address)).to.equal(
        true
      );
      expect(await nft.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.equal(
        false
      );
  ...
  describe("Reveal", function () {
    it("Should correctly request and receive randomness", async function () {
      const { owner, governor, mockToken, mockVRFWrapper, nft } =
    ...
    it("Should correctly assign traits probability", async function () {
      ...
      const traits = await Promise.all(
        Array.from({ length: totalQuantity }, (_, i) => i + 1).map((tokenId) =>
          nft.getTraits(tokenId)
        )
      );
      ...
      for (const trait of traits) {
        ...
        // Pompom rules check
        if (
          trait.traits.pompom != 0 &&
          ![2, 3, 4].includes(trait.traits.rarity)
        ) {
